...
def find_user_product(ids_products):
    ids = [product.id for product in ids_products]  # Extraer los IDs de los productos
    product_filter = Producto.objects.filter(id__in=ids)  # Consulta optimizada
    
    # Serializar todos los productos en una sola llamada
    serializer = ProductoSerializer(instance=product_filter, many=True)
